...
To address these challenges, this paper presents an openly available cross-domain dataset framework that supports both practical and generated datasets: practical data are organised through network telemetry and semantic abstraction, whereas synthetic data are demonstrated through an LLM-driven multi-agent DT workflow. Cross-domain is an architectural property of the framework: the optical-transport, fibre-sensing, and wireless-access domains connect to common repository, semantic-processing, and training interfaces. Experimental-platform usage required the practical datasets to be acquired in different time periods, but the subsequent learning workflow uses a unified data interface to invoke domain-specific preprocessing and pass optical and wireless features to a common model. The DT workflow further translates high-level requests into executable optical and wireless simulation tasks.
...
Figure~\ref{fig:proposed architecture.png} presents the cross-domain system architecture, building on our previous telemetry work~\cite{shen2025telemetry,Shen2026OFCdata}. Its physical-network layer depicts a general optical-transport network serving wireless-access domains, while the upper layers implement common repository, semantic-processing, and DT interfaces. These interfaces allow the framework to collect, store, retrieve, and process data from the supported domains through a common workflow.
...
In this paper, the implemented path comprises telemetry export, dataset documentation, unified data-interface calls, optical--wireless semantic evaluation, orchestrated task construction, and GNPy/ns-3 execution. The data interface invokes the appropriate optical or wireless preprocessing routine and exposes the resulting features through a common training call. AI engines and network controllers remain contextual integration points; QoT prediction, path optimisation, and physical-network enforcement by those blocks are not evaluated.
...
Under the telemetry architecture, we openly provide optical-transport, fibre-sensing, and wireless-access datasets~\cite{Shen_dataset_2025}. Experimental-platform usage constraints required these datasets to be collected in different time periods. This acquisition schedule does not change the cross-domain system architecture: the released files retain their source timestamps, while the subsequent training pipeline accesses them through the unified data interface described in Section~IV-B. Table~\ref{tab:dataset_summary} summarises their scope and machine-readable files. The datasets and associated metadata released in the practical-data and revision repositories are licensed under Creative Commons Attribution--NonCommercial 4.0 International (CC BY-NC 4.0), which permits sharing and adaptation with attribution for non-commercial purposes. We therefore describe these resources as openly available. Software and identified third-party material retain their own terms. A Croissant~1.0-compatible JSON-LD record documents the released data, access locations, sizes, checksums, and source-defined units. The final synthetic wireless bundle is available under \texttt{W4C\_THREE\_RAT\_SWEEP/}~\cite{Shen_dataset_2026}.
