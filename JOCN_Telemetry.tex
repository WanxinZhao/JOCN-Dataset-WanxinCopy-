...
Synthetic three-RAT wireless DT & 48 configurations; 432 application flows; 10-s runs & NR/Wi-Fi/LiFi scenario settings, per-flow KPIs, LiFi optical state & CSV + XML/JSON & CC BY-NC 4.0 \\
...
For reproducibility, a separately labelled instrumented run used \texttt{gpt-4o-mini-2024-07-18}, temperature 0.2, a 90-s timeout, at most three attempts per call, and strict mode. Planner, Scenario Expansion, and Reflection were each called twice. All six API calls succeeded on their first attempt, using 108,308 tokens, 26.22~s summed API latency, and an estimated \$0.01727 in text-token cost. Complete prompts, responses, logs, recovery history, and source/input hashes are released at commit \texttt{82914127b70acf3b220bc09a56a179cb7abd1252}~\cite{JOCN_revision_code_2026}. The trace also records manual recovery during simulator execution, so the reported measurements characterise one fully documented workflow run.
...
\begin{figure*}[t]
    ...
    \captionsetup{justification=centering}
    \caption{GNPy data-generation results over the NDFF topology. (a) Configuration enumeration and execution-history counts; (b) GSNR distributions for the two modulation/slot-width settings at $-5.5$~dBm; (c) route-dependent mean GSNR change across the launch-power sweep; and (d) QPSK mean GSNR versus active-channel count. Numerical panels use 6,120 valid unique configurations; 24 all-off cases are excluded from metric summaries. GSNR is measured over the signal bandwidth.}
    \label{fig:usecase1}
    \vspace{-0.3cm}
\end{figure*}
...
Figure~\ref{fig:usecase1}(b)--(d) uses the unique refined-pass records. At $-5.5$~dBm, mean QPSK GSNR decreases from 26.34 to 22.57~dB on Bristol--Bradley Stoke and from 9.52 to 3.80~dB on Bristol--Powergate as active-channel count rises from one to eight. Launch-power sensitivity is route dependent: over the tested range, the Bradley Stoke mean decreases whereas the Powergate mean increases slightly. The sweep therefore characterises route-specific sensitivity across the three tested operating points.


For numerical reproducibility, we independently replayed 96 cases stratified by path, joint modulation/slot-width setting, power, and channel load. A pinned GNPy v2.12 compatibility environment completed all 96 cases and reproduced the archived GSNR values within 0.00050~dB. A schema-corrected GNPy v2.14.2 environment also completed all cases but differed from the archive by 0.628~dB on average and 1.435~dB at most. The version sensitivity is larger on multi-span routes. The retained configuration warnings identify the remaining physical-model calibration work.
...
Overall, the use case converts a high-level request into structured simulator artefacts and a reflection-proposed refined sweep. The deterministic baseline reproduces the fixed configuration set, while the LLM roles add request interpretation and refinement based on the first-pass KPI summary. Evaluation over larger topologies, repeated requests, and calibrated equipment models remains future work.


\subsubsection{ns-3-Based Multi-Access Network Performance Data Generation}
\label{sec:ns3_wireless}

The LLM-driven workflow constructs and executes a bounded three-access synthetic wireless scenario at the same technology-class level as the practical multi-access testbed: 5G NR, Wi-Fi~6/IEEE~802.11ax, and LiFi. The generated dataset is separate from the measured wireless telemetry and evaluates configurable data generation and workflow interoperability. Physical-testbed calibration is outside this synthetic scenario.
...
The LiFi branch uses an equation-driven LOS LiFi/OWC link model coupled to the stock ns-3 point-to-point IP/UDP access abstraction. It computes Lambertian emission, transmitter/receiver geometry, receiver FOV, received optical power, photodetector responsivity, shot and thermal noise, electrical SNR, OOK BER, and packet-error probability, which drives packet corruption. The released configuration records all model parameters used in the synthetic scenario.
...
\end{tabularx}
\vspace{2pt}
\footnotesize The validation levels address implementation consistency, numerical agreement, flow mapping, fixed-seed repeatability, and sweep completeness for the declared scenario.
\end{table}

Under the adopted simulation configuration, increasing the offered load from 2 to 20~Mbps increased run-level throughput by 18.486~Mbps for NR and 15.726~Mbps for LiFi. For Wi-Fi 802.11ax, the corresponding throughput increase was 0.151~Mbps, while mean delay increased by 1,653.88~ms and the FlowMonitor-reported packet-loss ratio increased by 0.701. These results indicate a configuration-specific queueing response in the Wi-Fi branch and load-responsive throughput in the NR and LiFi branches. Increasing mobility from 0.5 to 3~m/s changed throughput by 0~Mbps for NR, 0.00018~Mbps for Wi-Fi 802.11ax, and $-2.572$~Mbps for LiFi under the selected geometry and channel models. Narrowing the LiFi FOV from 70$^\circ$ to 40$^\circ$ reduced the in-FOV fraction by 0.299 and the received-packet fraction by 0.299 in matched run-level comparisons. The ideal concentrator gain also varies with FOV, so the aggregate $H_{\mathrm{LOS}}$ and SNR responses depend on both receiver acceptance and concentrator gain. All branch comparisons refer to the topology, channel/helper settings, link rates, delays, load, mobility, and LiFi model declared above.
...
\paragraph{Wireless Scope and Limitations.} The final synthetic backend uses 5G NR via CTTC 5G-LENA, Wi-Fi 802.11ax, and an equation-driven LOS LiFi/OWC model. The LiFi implementation couples the LOS equations to a stock packet-access abstraction and has not been calibrated to the practical testbed. Accordingly, the reported technology responses apply to the declared configuration. The validation covers implementation consistency and reproducibility of the bounded synthetic telemetry.
