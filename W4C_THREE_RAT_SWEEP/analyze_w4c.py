#!/usr/bin/env python3
"""Scientific, run-aware analysis of the frozen W4C synthetic dataset."""
...
def write_fallback_svgs(run_rows: list[dict[str, object]], physical_rows: list[dict[str, object]]) -> list[str]:
    ...
    line_chart("throughput_vs_offered_load.svg", "throughput_mbps", "W4C synthetic wireless throughput", "Throughput (Mbps)")
    line_chart("delay_vs_offered_load.svg", "mean_delay_ms", "W4C synthetic wireless delay", "Mean delay (ms)")
    ...
    parts = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{width}" height="{height}" viewBox="0 0 {width} {height}"><rect width="100%" height="100%" fill="white"/><text x="350" y="24" text-anchor="middle" font-family="sans-serif" font-size="17">W4C LiFi/OWC FOV response</text>']
    panel_w = chart_w / 2
...
def optionally_plot(run_rows: list[dict[str, object]], physical_rows: list[dict[str, object]]) -> list[str]:
    ...
    plt.title("W4C synthetic wireless throughput")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
    ...
    plt.title("W4C synthetic wireless delay")
    plt.grid(alpha=0.25)
    plt.legend()
    plt.tight_layout()
    ...
    fig.suptitle("W4C LiFi/OWC physical response")
    fig.tight_layout()
