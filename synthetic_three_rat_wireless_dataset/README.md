# Synthetic three-RAT wireless dataset

This directory contains a bounded synthetic wireless dataset generated with
three access branches. It is a simulation dataset, not a measurement from the
practical wireless testbed.

The dataset corresponds to an experimental three-RAT backend. The separate
upstream framework repository's `NS3_Wireless/` directory is an earlier
LTE/EPC, IEEE 802.11n, and CSMA-surrogate baseline and is not the source of
this dataset.
...
- `final_runs/` contains representative integrated-run and repeatability
  evidence.
- `ns3/`, `config/`, `aggregate_dataset.py`, and `analyze_dataset.py` contain
  the project-local source, configuration, and aggregation/analysis scripts.
- `lifi_validation/` contains the independent mathematical reference and
  LiFi equation checks.
- `dataset_validation_report.md` provides the validation and claim boundaries.
