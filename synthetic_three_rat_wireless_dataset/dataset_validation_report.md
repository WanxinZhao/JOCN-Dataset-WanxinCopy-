# Validation of the bounded synthetic three-RAT wireless dataset
...
Evidence level: the structural and numerical statements below are **OBSERVED** in the released outputs or source. Interpretations about load, mobility, and FOV effects are **INFERRED from the bounded sweep** and are explicitly not generalized beyond the adopted configuration. Practical-testbed calibration is **MISSING**.
...
The project-local source is:

- `synthetic_three_rat_wireless_dataset/ns3/three_rat_smoke.cc`
- `synthetic_three_rat_wireless_dataset/ns3/lifi_owc.h`
- `synthetic_three_rat_wireless_dataset/ns3/lifi_owc_impl.inc`
- `synthetic_three_rat_wireless_dataset/ns3/lifi_owc.cc`

The executable source uses the project-local implementation include at `three_rat_smoke.cc:35-38`; no external absolute source include is required. The small `.cc` wrapper at `lifi_owc.cc:1-2` includes the local implementation file. The source remains separate from the validated legacy project.
...
| NR backend | CTTC 5G-LENA `5g-lena-v4.0.y` | `three_rat_smoke.cc:340`; qualified execution environment |
| Qualified 5G-LENA revision | `2c0ed9740e677224bdfff4eab5aea5adcd8f6052` | backend qualification record |
| Build root | `/tmp/w4a-ns3-EEnH/ns-3.44` | archived execution record |
| Binary | `/tmp/w4a-ns3-EEnH/ns-3.44/build/scratch/ns3.44-three_rat_smoke` | archived sweep command/status |
| Build profile | CMake `release`, C++20 | ns-3 CMake cache in the qualified build root |
| Compiler | g++ 15.2.0 | `/usr/bin/g++ --version` |
| Build command | `/home/ubuntu/.local/bin/python3.12 ns3 build -j 4 three_rat_smoke` | archived execution record |
...
The preflight used load 5 Mbps, speed 0.5 m/s, seed 1, the final geometry, 10 s simulation, and the 1.0–9.5 s application window. The output is `synthetic_three_rat_wireless_dataset/preflight/fov_preflight_summary.csv`, with the interpretation in `preflight/fov_preflight_report.md`.
...
The archived sweep record contains 48 PASS runs, zero nonzero return codes,
and 48 matched-flow counts of 9. Total recorded execution time was
approximately 226.49 s; this is not a scientific KPI. The released aggregate
tables preserve the 48-run application-flow and structural results.
...
An independent metadata/KPI/mapping tuple comparison found no mismatched or missing five-tuples in any of the 48 runs. Required numeric fields in both KPI and optical-state files contained no NaN or Inf values. These checks were performed by `aggregate_dataset.py` and by the additional metadata/KPI/mapping comparison recorded during dataset generation.
...
The independent Python test suite is `synthetic_three_rat_wireless_dataset/lifi_validation/lifi_reference.py` and `run_lifi_unit_tests.py`; it does not call the C++ implementation. The report `lifi_unit_test_report.md` records **9/9 PASS** for Lambertian order, aligned LOS gain, inverse-square behavior, FOV gate, received power, finite zero-power noise/SNR, monotonic PER, and BER/PER bounds.
...
They are analysis candidates only; no LaTeX or manuscript figure was edited during dataset generation. The proposed compact manuscript table is a parameter-sweep/dataset-coverage table with the following entries:
...
This row must be kept distinct from the measured practical wireless dataset. These files contain generated ns-3 telemetry, not physical measurement records.

## 12. Legacy and practical-data boundary

The old LTE/EPC + Wi-Fi 802.11n + CSMA-surrogate sweep remains an unchanged legacy baseline under its existing project path. It was not numerically mixed with the three-RAT dataset, and its earlier validation was not reused as validation of the new stack.

The released three-RAT dataset demonstrates configurable synthetic scenario construction and export for the access-technology classes of interest. It does not demonstrate that:
...
## 13. Dataset validation status

The generation process did not run a sweep larger than 48 configurations and did not edit the manuscript. The source is suitable for manuscript integration only with the above claim boundaries and with the evidence table and analysis artifacts cited as generated-data validation, not physical validation.

```text
DATASET_VALIDATION_VERDICT=PASS_WITH_LIMITATIONS
